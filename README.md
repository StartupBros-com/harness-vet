...
Large list-candidates receive whole-list triage and at most three deep-vets by default; interactive runs can choose lighter sizing. Without an explicit landing instruction, use discovered target conventions, then PR-for-review. An unresolved destination still authorizes no installation.

## Behavioral fixtures

The eight authored [scope cases](tests/scope_cases.json) cover global versus project fit, callable incumbents, unavailable copies, retained security findings, multiple targets, unresolved destinations and both plugin delivery choices. [Fixture instructions](tests/README.md) explain structural validation and evaluation with a fresh model session per case, withholding `expected` and `forbidden_claims` from the evaluated model. The final single-run smoke check passed all eight cases with an independent semantic review; the rejected draft failed the negative control. [Recorded responses and limits](tests/scope-smoke-final.json) preserve the results, and [the initial failures](tests/scope-smoke-results.json) remain visible. These synthetic decisions do not establish runtime isolation, estate-planning quality, statistical improvement or release deployment.
